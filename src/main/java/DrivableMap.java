...
class DrivableMap {
     ...
     *       and returns true if there is at least one item in drivable_map
    ...
    public List<Tradable> getTradable(){
        ...
        for (Drivable obj : drivable_map.values()){
            if (obj instanceof Tradable){
                Tradable object = (Tradable)obj;
                if (object.getPrice() != Tradable.MISSING_PRICE){
                    tradableList.add(object);
                }
            }
        }
        return tradableList;
